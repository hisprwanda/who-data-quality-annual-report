...
export const getSelectedOrgUnitMoreInformation =  {
  
    results: {
      resource: `organisationUnits/qICVQ5VD0Y7.json`,
      params: {
        fields: ["children[displayName,id,level,children::isNotEmpty]"],
      },
    }
};

export const processOrgUntInformation = (id) => {
    const results = {
        resource: `organisationUnits/${id}.json`,
        params: {
          fields: ["children[displayName,id,level,children::isNotEmpty]"],
        }
      }
...
export const loadAnalytics = {
  results: {
    resource:
      "analytics.json?dimension=pe:2000W01&filter=ou:Hjw70Lodtf2;LEVEL-2&displayProperty=NAME&skipData=true",
    paging: true,
  },
};
...
export const loadOrganizationUnit = (orgUnit) => {
  return axios.get(
    `${baseURL}/${subURL}/api/organisationUnits/${orgUnit}.json?fields=children[displayName,id,level,children::isNotEmpty]`,
    {
      auth: userAuth,
    }
  );
...
export const loadReportingRate = (dx, ou, level, pe) => {
  return axios.get(
    `${baseURL}/${subURL}/api/analytics.json?dimension=dx:${dx[0]}.REPORTING_RATE&dimension=ou:${ou}&dimension=pe:2022&displayProperty=NAME`,
    {
      auth: userAuth,
    }
  );
...
export const loadDatasetInformation = (dataset) => {
  return axios.get(
    `${baseURL}/${subURL}/api/dataSets.json?filter=id:in:[${dataset}]&fields=displayName,id,periodType&paging=false`,
    {
      auth: userAuth,
    }
  );
...
export const loadReportingRateOnTime = (dx, ou, level) => {
  return axios.get(
    `${baseURL}/${subURL}/api/29/analytics.json?dimension=dx:${dx}.REPORTING_RATE_ON_TIME&dimension=ou:${ou}&dimension=pe:2022&displayProperty=NAME`,
    {
      auth: userAuth,
    }
  );
...
export const loadOrganizationUnitGroups = () => {
  return axios.get(
    `${baseURL}/${subURL}/api/29/organisationUnitGroups.json?=&paging=false`,
    {
      auth: userAuth,
    }
  );
...
export const loadOrganizationUnitLevels = () => {
  return axios.get(
    `${baseURL}/${subURL}/api/filledOrganisationUnitLevels.json?fields=id,displayName,name,level`,
    {
      auth: userAuth,
    }
  );
