...
export const deleteNumerator = (configurations, numeratorToDelete) => {
    const metaDataVersion = configurations.metaDataVersion
    const numerators = configurations.numerators
    const coreIndicators = configurations.coreIndicators
    const dataSets = configurations.dataSets
    ...
    const configurationsToSave = {
        metaDataVersion,
        numerators: numerators.filter(
            (numerator) => numerator.code !== numeratorToDelete.code
        ),
        coreIndicators: coreIndicators.filter(
            (coreIndicator) => coreIndicator !== numeratorToDelete.code
        ),
        dataSets,
        denominators,
        denominatorRelations,
        ...
        groups: groups.map((group) => {
            group.members = group.members.filter(
                (member) => member !== numeratorToDelete.code
            )
            return group
        }),
    }
